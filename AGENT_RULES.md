...
`memory-bank/runtime/active-task.md` 的 `Current Status` 必须恰好是以下小写值之一：`pending`、`active`、`blocked`、`completed`、`aborted`。禁止使用自然语言别名、标点变体或 checklist 推断状态；未知值必须先修正，不能静默归档。

归档前先运行 `python .paradigma/tools/pd-archive-task.py --dry-run` 审查 mutation plan；确认后才使用 `--write`。归档成功后 active-task 重置为 `pending`。
...
- 根 `index.md` 的高层导航由人工维护；子目录 generated block 与 `.paradigma/cache/` 机器索引由 `pd-index.py` 维护，不得手改。
...
7. 新增、删除或修改 concept 元数据后运行 `python .paradigma/tools/pd-index.py rebuild`，再运行 `pd-index.py verify`；根导航只在高层入口变化时人工更新。
8. 运行生产检查：`python .paradigma/tools/pd-check-all.py`
9. 任务状态为 `completed` 时，先运行 `python .paradigma/tools/pd-archive-task.py --dry-run` 审查 mutation plan，再按需使用 `--write`；progress logs 过长时可运行 `python .paradigma/tools/pd-compact-progress.py --write`。
10. **（可选）若项目存在 `DESIGN.md`** → 检查视觉设计规范与实际实现的一致性，必要时更新 DESIGN.md。
11. 根据 `conventions.md` 评估版本号；需要时更新 `VERSION` 与 `memory-bank/logs/changelog.md`。
12. 结束时告知用户："Memory-bank 已更新完毕。本次更新了：[文件列表]"。
