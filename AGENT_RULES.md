...
8. 任务状态为 `completed` 时，先运行 `python .paradigma/tools/pd-archive-task.py --dry-run` 审查 mutation plan，再按需使用 `--write`；progress logs 过长时可运行 `python .paradigma/tools/pd-compact-progress.py --write`。
